  ...
  if (command != 0)
  {
    ...
    if ( micros() - local > 0.032*1.0e6)
    {
      ...
      if(t_d == 0)
      {
        ...
        unsigned long localT_ = micros();
        while (micros() - localT < 1.5*1.0e6)
        {
          if ( micros() - localT_ > 0.027*1.0e6)
          {
            Stagation();
            goStraight(5.0);
...
void beacon_detect(void)
{
  ...
  //---------------------Round 1---------------------//
  if (command == 1) // Right & middle
  {
    unsigned long init_1 = micros();
    while ( micros() - init_1 <= 5.0*1.0e6)
    { 
      ...
      //if (ratio == 0.30 || ratio == 0.15)
      if (ratio >= 0.01 || ratio >= 0.02)
      {
        Stop();
        break;
      }
  ...
  currT = micros();
  unsigned long init_3 = micros();
  while (micros()-init_3 <= 13.0*1.0e6)
  {
    if ( micros() - currT > 0.012*1.0e6)
    {
      Stagation();
